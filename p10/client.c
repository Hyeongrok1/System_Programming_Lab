...
int main (int argc, char *argv[]) {
	...
	/* Create socket */
	if ((connecting_fd = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
	...
	if ((host_entry = gethostbyname(host)) == NULL) {
		printf("Client: invalid host name %s\n", host); 
		exit(3);
	} 
	memset((char *)&socket_address, 0, sizeof(socket_address));
	socket_address.sin_family = AF_INET; 
	memcpy((char *)&socket_address.sin_addr.s_addr, (char *)host_entry->h_addr_list, host_entry->h_length);
